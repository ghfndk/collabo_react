...
import { Button, Col, Container, Row, Table, Image, Form } from "react-bootstrap";
...
function App({ user }) {
    ...
    // user?.id : Optional Chaning(물음표를 적어 주면 오류가 발생하지 않고 undefined를 반환해 줍니다.)
    ...
    const toggleAllCheckBox = (isAllCheck) => {
        ...
        setCartProducts((previous) => {
            // 모든 객체(카트 상품)들이 나머지 속성은 보존하고, 체크 상태(checked)를
            ...
            // 비동기적 렌더링 문조레 수정된 updatedProducts 항목을 매개 변수로 넘겨야 정상적으로 동작합니다.
    ...
    const refreshOrderTotalPrice = (product) => {
        let total = 0; // 총 금액 변수

        product.forEach((bean) => {
    ...
    const changeQuantity = async (cartProductId, quantity) => {
        ...
        try {
            ...
            // 스플링의 WebConfig 클래스안의 addCorsMappings() 메소드를 참조하시길 바랍니다.
    ...
    const deleteCartProduct = async (cartProductId) => {
        ...
        if (isConfirmed) {
            ...
            try {
                ...
                setCartProducts((previous) => {
                    const updatedProducts = previous.filter((bean) =>
                        bean.cartProductId !== cartProductId);
    ...
    return (
        <Container className="mt-4">
            ...
            {/* 좌측 정령(text-start), 가운데 정령(text-center), 우측 정령(text-end)*/}
