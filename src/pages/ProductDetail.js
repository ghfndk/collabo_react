...
function App({ user }) {
    ...
    const [product, setProduct] = useState(null); // 백앤드에서 넘어온 상품 정보
    // 로딩 상태를 의미하는 stata로, 값이 true이면 현재 로딩 중입니다. 
    ...
    // 피라미터 id가 갱신이 되면 화면을 다시 rendering 시킵니다.
    useEffect(() => {
    ...
    // 아직 backand에서 읽어 오지 못한 경우를 대비한 코딩입니다.
    if (loading === true) {
        return (
                        ...
                        <Card.Body>
                            ...
                            <Table striped>
                                <tbody>
                                    ...
                                    <tr>
                                        <td className="text-center">등록 일자</td>
                            ...
                            {/* 구매 수량 입력란 */}
                            {/* as={Row}는 랜더링시 기본 값인 <div> 말고 Row로 랜더링하도록 해줍니다.  */}
