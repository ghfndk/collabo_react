...
테스트 시나리오
    이미지 폴더에 "product_"로 시작하는 새로운 이미지가 업로드되어야 합니다.
    데이터 베이스에 이전 행의 정보가 수정이되어야 합니다.
...
function App() {
    ...
    const initial_vaulue = {
        name: '', price: '', category: '', stock: '', image: '', description: ''
    }; // 상품 객체 정보

    // product는 백앤드에게 넘겨줄 상품 등록 정보를 담고 있는 객체
    const [product, setProduct] = useState(initial_vaulue);
    ...
    const FileSelect = (event) => {
        ...
        // FuleReader는 웹 브라우저에서 제공해주는 내장 객체로, 파일 읽기에 사용가능합니다.
    ...
    return (
        <Container>
            ...
            <Form onSubmit={SubmitAction}>
                ...
                {/* 이미지 type="file"이어야 하고, 이벤트 처리 함수를 별개로 따로 만들도록 합니다. */}
                <Form.Group className="mb-3">
